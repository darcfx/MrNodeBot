...
class IrcWrappers {
    ...
    async handleCommands(from, to, text, message) {
        ...
        // The following administration levels piggy back on services, thus we check the acc status of the account and defer
            ...
            switch (this.app.Config.nickserv.authType) {
                case 'status':
     ...
     */
    async handleAuthenticatedCommandsStatus(nick, to, text, message) {
        const normalizedText = IrcWrappers._normalizeText(text);
     ...
     * @returns {boolean} command status
     */
    async handleAuthenticatedCommandsAcc(nick, to, text, message) {
        const normalizedText = IrcWrappers._normalizeText(text);
        console.log(normalizedText);

        // Parse vars
        // Darc ACC 3
        // const [user, acc, code] = normalizedText.replace('ACC', '').split(' ');
        // This is tied to Liera.Chat
        const nickservMatches = normalizedText.match(/(\w+) ACC (\d)/);
